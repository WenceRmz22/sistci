<?php
		...
		$nuevafecha = strtotime ( '-8 day' , strtotime ( $FechaFinal ) ) ;
		$FechaInicial   = date ( 'Y-m-j' , $nuevafecha );
		$tipop          =  $_GET['tipop'];
		$NoCliente      = base64_decode( $_GET['txtClave']);
		$Aduana         =  $_GET['AduSec'];
		$array1         = array();
		$array2         = array();
		$array3         = array();
		...
		if($array1)
		{
			...
			$objPHPExcel->getActiveSheet()->getColumnDimension('AL')->setWidth(100);
			...
			->setCellValue('P3','AD VALOREM - Tip. Pago')
			->setCellValue('Q3','D.T.A')
			->setCellValue('R3','D.T.A - Tip. Pago')
			->setCellValue('S3','I.V.A')
			->setCellValue('T3','I.V.A - Tip. Pago')
			->setCellValue('U3','PREV')
			->setCellValue('V3','PREV - Tip. Pago')
			->setCellValue('W3','OTROS IMP.')
			->setCellValue('X3','OTROS IMP - Tip. Pago')
			->setCellValue('Y3','PROVEEDOR')
			->setCellValue('Z3','FACTURA')
			->setCellValue('AA3','FECHA PAG.')
			->setCellValue('AB3','TIP. CAMBIO')
			->setCellValue('AC3','COVE')
			->setCellValue('AD3','TAX-ID')
			->setCellValue('AE3','ID PAIS')
			->setCellValue('AF3','REGIMEN')
			->setCellValue('AG3','FIRMA DE VALIDACION.')
			->setCellValue('AH3','FIRMA DE PAGO')
			->setCellValue('AI3','MEDIO DE TRANSPORTE')
			->setCellValue('AJ3','IDENTIFICADOR')
			->setCellValue('AK3','COMPLEMENTO')
			->setCellValue('AL3','OBSERVACIONES');
			//DATOS EN ARRAYS
			$i = 4;
			$fc = 0;
			for($d = 0;$d < count($array1);$d++ ){
				...
				->setCellValue('I'.$i,$array1[$d]['FECHAPAGO'])
				->setCellValue('J'.$i,$array1[$d]['CLAVE'])
				->setCellValue('M'.$i,$array1[$d]['VALORCOMERCIAL'])
				->setCellValue('N'.$i,$array1[$d]['INCREMENTABLE'])
				->setCellValue('AB'.$i,$array1[$d]['TIPODECAMBIO'])
				->setCellValue('AF'.$i,$array1[$d]['REGIMEN'])
				->setCellValue('AG'.$i,$array1[$d]['FIRMAVALIDACION'])
				->setCellValue('AH'.$i,$array1[$d]['FIRMAPAGO'])
				->setCellValue('AI'.$i,$array1[$d]['MEDIOTRANSPORTE']);
				...
				for($f = 0; $f < count($array2);$f++){
					if($array1[$d]['IdTemporal'] == $array2[$f]['Id05']){
						$objPHPExcel->setActiveSheetIndex(0)
						->setCellValue('Y'.$fc,$array2[$f]['PROVEEDOR'])
						->setCellValue('Z'.$fc,$array2[$f]['FACTURA'])
						->setCellValue('AA'.$fc,$array2[$f]['FECHAFACTURA'])
						->setCellValue('AC'.$fc,$array2[$f]['COVE'])
						->setCellValue('AD'.$fc,$array2[$f]['TAXID'])
						->setCellValue('AE'.$fc,$array2[$f]['IDPAIS']);
						$fc++;
					}
				}
				for($des = 0; $des < count($array3);$des++){
					if($array1[$d]['IdTemporal'] == $array3[$des]['Id16']){
						...
						->setCellValue('K'.$ds,$array3[$des]['PV'])
						->setCellValue('L'.$ds,$array3[$des]['PO']);
